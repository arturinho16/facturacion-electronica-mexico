...
const cargarXsltCadenaOriginal = () => {
  const base = readFileSync(xsltPath, 'utf8');
  const nomina = readFileSync(join(process.cwd(), 'src/lib/sat/nomina12.xslt'), 'utf8');
  const nominaTemplates = nomina
    .replace(/^[\s\S]*?<xsl:stylesheet\b[^>]*>/, '')
    .replace(/<\/xsl:stylesheet>[\s\S]*$/, '')
    .trim();

  return base.replace(/\s*<xsl:include\s+href="(?:http:\/\/www\.sat\.gob\.mx\/sitio_internet\/cfd\/nomina\/nomina12\.xslt|nomina12\.xslt)"\s*\/>/, `\n${nominaTemplates}\n`);
};
const xsltContent = cargarXsltCadenaOriginal();
...
export function normalizarObjetoImp(value: unknown): '01' | '02' | '03' | '04' {
  const texto = String(value ?? '').trim().toUpperCase();
  const match = texto.match(/^0?([1-4])$/);
  if (match) return (`0${match[1]}`) as '01' | '02' | '03' | '04';

  const encabezado = texto.match(/^0?([1-4])\b/);
  if (encabezado) return (`0${encabezado[1]}`) as '01' | '02' | '03' | '04';

  if (texto.includes('SIN OBJETO')) return '01';
  if (texto.includes('OBJETO NO OBLIGADO')) return '03';
  if (texto.includes('OBLIGADO')) return '02';
  if (texto.includes('OBJETO')) return '02';
...
export function generarXMLUnsigned(datos: DatosFactura, noCertificado: string, certificadoB64: string): string {
  ...
  if (!datos.emisor?.rfc || !datos.emisor.nombre || !datos.emisor.regimenFiscal) {
  ...
  root.ele('cfdi:Emisor', { Rfc: datos.emisor.rfc.trim(), Nombre: datos.emisor.nombre.trim(), RegimenFiscal: datos.emisor.regimenFiscal.trim() }).up();
  ...
  for (const c of datos.conceptos) {
    const objetoImp = normalizarObjetoImp(c.objetoImpuesto);
    ...
    if (objetoImp !== '01' && (c.ivaImporte > 0 || c.iepsImporte > 0)) {
...
export async function timbrarFactura(datos: DatosFactura): Promise<{ uuid: string; xmlTimbrado: string; noCertificadoSAT: string; }> {
  ...
  const cerB64 = credentials?.certificadoBase64;
  const keyB64 = credentials?.llaveBase64;
  ...
  if (!usuario || !passwordFinkok) {
    throw new Error('Configura el usuario y contraseña del PAC antes de timbrar.');
  }
  if (!config?.rfc || !config.razonSocial || !config.regimenFiscal) {
    throw new Error('Guarda primero el perfil fiscal del emisor.');
  }
  ...
  const xmlSinSello = generarXMLUnsigned(
    {
      ...datos,
      lugarExpedicion: datos.lugarExpedicion || config.codigoPostal,
      emisor: {
        rfc: config.rfc,
