...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const c of conceptos) {
      ...
      const iva = objetoImpuesto !== '01' ? baseImpuesto * Number(c.ivaTasa) : 0;
      const ieps = objetoImpuesto !== '01' ? baseImpuesto * Number(c.iepsTasa) : 0;
      ...
      data: {
        ...
        conceptos: {
          create: conceptos.map((c: any) => {
            ...
            const ivaImporte = objetoImpuesto !== '01' ? baseImp * Number(c.ivaTasa) : 0;
            const iepsImporte = objetoImpuesto !== '01' ? baseImp * Number(c.iepsTasa) : 0;
