...
import { Search, Plus, UploadCloud, Users, ArrowLeft, Download, Loader2, Edit, FileText, Trash2 } from "lucide-react";
...
export default function EmpleadosPage() {
    ...
    const handleDownloadTemplate = () => {
        const headers = "Nombre,ApellidoPaterno,ApellidoMaterno,CURP,NSS,RFC,Calle,Colonia,NumExterior,NumInterior,CP,Localidad,Municipio,Estado,Email,Grupo,Sucursal,FechaRelacionLaboral,Salario,SalarioCuotas,Contrato,RegimenContratacion,RiesgoPuesto,TipoJornada,Banco,CLABE,Periodicidad,Departamento,Puesto,NumEmpleado\n";
    ...
    const handleExportCSV = () => {
        ...
        const csvRows = selectedData.map(emp => [
            ...
            emp.curp || '',
            emp.nss || '',
            Number(emp.salario || 0).toFixed(2),
            `"${emp.puesto || ''}"`,
            `"${emp.departamento || ''}"`
        ]);
    ...
    const deleteEmpleados = async (ids: string[]) => {
        if (!ids.length) return;
        const label = ids.length === 1 ? "este empleado" : `${ids.length} empleados seleccionados`;
        if (!confirm(`¿Eliminar ${label}? También se eliminarán sus recibos de nómina relacionados.`)) return;
        ...
        try {
            ...
            if (!res.ok) {
                alert(data.error || "No se pudieron eliminar los empleados.");
                return;
            }

            alert(`Empleados eliminados: ${data.empleadosEliminados || 0}`);
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                    ...
                    <div className="p-5 border-b border-slate-100 flex flex-col xl:flex-row items-center justify-between gap-4 bg-slate-50">
                        ...
                        <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto">
                            ...
                            <button
                                onClick={() => deleteEmpleados(selectedIds)}
                                disabled={selectedIds.length === 0 || isDeleting}
                                className={`flex items-center gap-1 font-bold text-sm px-4 py-2.5 rounded-xl transition-colors whitespace-nowrap ${selectedIds.length === 0 || isDeleting ? 'bg-slate-100 text-slate-400 border border-slate-200 cursor-not-allowed' : 'bg-white border border-rose-600 text-rose-600 hover:bg-rose-50'}`}
                            >
                                    ...
                                    currentEmpleados.map((emp) => (
                                        <tr key={emp.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-center">
                                                ...
                                                <button
                                                    type="button"
                                                    onClick={() => deleteEmpleados([emp.id])}
                                                    disabled={isDeleting}
                                                    className="inline-flex items-center justify-center p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors disabled:opacity-50"
