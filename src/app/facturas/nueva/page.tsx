...
function NuevaFacturaForm() {
  ...
  const resetForm = async () => {
    ...
    await cargarSiguienteFolio(nuevaInfoCDMX.serieStr);
  };
  ...
  const handleRevisar = async () => {
    ...
    const payload = {
      ...
      retencionIVAPct,
      retencionISRPct,
      emisorCp: emisorConfig.cp.match(/\d{5}/)?.[0],
  ...
  const handleTimbrarDesdeModal = async (idFactura: string) => {
    try {
      ...
      try {
        ...
        const qrUrl = `https://verificacfdi.facturaelectronica.sat.gob.mx/default.aspx?id=${uuidFinal}&re=${emisorConfig.rfc}&rr=${rr}&tt=${totalStr}&fe=${sello8}`;
                ...
                {c.productoId && (
                  <div className="grid grid-cols-2 md:grid-cols-6 gap-3 pt-2 border-t border-slate-200">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="text-sm font-mono bg-orange-50 text-orange-600 p-2 rounded-lg">
                        {fmt(iva)}
                      </div>
                    </div>
          ...
          <div className="bg-slate-50 rounded-2xl p-4 space-y-2 border border-slate-200">
            ...
            {retencionIVA > 0 && (
              <div className="flex justify-between text-sm text-red-500">
                ...
                <span className="font-mono">-{fmt(retencionIVA)}</span>
              </div>
            )}
            ...
            {retencionISR > 0 && (
              <div className="flex justify-between text-sm text-red-500">
                ...
                <span className="font-mono">-{fmt(retencionISR)}</span>
              </div>
            )}

            <div className="flex justify-between text-2xl font-bold text-blue-900 border-t pt-2 mt-2">
