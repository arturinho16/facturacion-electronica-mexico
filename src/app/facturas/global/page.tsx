...
export default function FacturaGlobalPage() {
  ...
  const [codigoPostalEmisor, setCodigoPostalEmisor] = useState('');
  ...
  useEffect(() => {
    ...
    fetch('/api/configuracion', { cache: 'no-store' })
      ...
      .then(config => setCodigoPostalEmisor(config?.codigoPostal || ''))
      .catch(() => setCodigoPostalEmisor(''));
  }, []);
  ...
  const handleTimbrar = async () => {
    ...
    if (!codigoPostalEmisor) {
      alert("Configura el código postal del emisor antes de crear factura global."); return;
    }
    ...
    try {
      ...
      const payload = {
        serie: 'FAG', folio: folioUnico, fecha: hoy.toISOString(), lugarExpedicion: codigoPostalEmisor,
        moneda: 'MXN', tipoCambio: 1, formaPago: '01', metodoPago: 'PUE',
        tipoComprobante: 'I', usoCFDI: 'S01',
        receptorManual: { rfc: 'XAXX010101000', nombre: 'PUBLICO EN GENERAL', cp: codigoPostalEmisor, regimenFiscal: '616' },
                        ...
                        <div className="absolute z-10 w-full mt-1 bg-white border-2 border-indigo-200 rounded-xl shadow-xl overflow-hidden">
                          {sugerencias.map(prod => (
                            <div key={prod.id} onMouseDown={(e) => { e.preventDefault(); selectProduct(t.id, prod); }} className="p-3 hover:bg-indigo-50 cursor-pointer border-b border-indigo-50 last:border-0 flex justify-between items-center">
                              ...
                              <div className="text-sm font-bold text-slate-600">{fmt(Number(prod.precio) * (1 + Number(prod.ivaTasa)))}</div>
                            </div>
                          ))}
                        </div>
                      )}
